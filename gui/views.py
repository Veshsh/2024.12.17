...
def List(request, id=None):
    machins = (
        models.Machin.objects.filter(user=request.user, user_monitor_permision=True)
        | models.Machin.objects.filter(other_monitor_permision=True)
        | models.Machin.objects.filter(
            group__in=models.Group.objects.filter(users=request.user),
            group_monitor_permision=True,
        )
    )
    ...
    if request.method == "GET":
        ...
        if "id" in request.POST and id == 0:
            ...
            machins = models.Machin.objects.all().filter(
                machin_group__id=request.POST["group_filter"]
            )
            return List_render(request, machins)
...
def ssh(id, usr, cmd):
    ...
    if machin.log_save:
        f = open(fpl, "a")
        f.write(
            f"[{datetime.now().strftime('%a, %d %b %Y %H:%M:%S +0000')}]connect {usr} to {str(machin.ip.ip)}:{machin.port}\n"
        )
        f.close()
    print(log)
    return cmdout
